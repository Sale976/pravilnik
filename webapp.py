    ...
    st.markdown("""
        <style>
            ...
            section[data-testid="stSidebar"] .stButton:nth-of-type(1) button {
                ...
                left: 47px;       /* 10px od leve ivice ekrana/sidebar-a */
...
st.markdown(
    """
    <h2 style='text-align: center; margin-top: -60px; font-size:28px;'>
        Web aplikacija za pretragu Pravilnika o Podeli Motornih Vozila (PPMV) i Pravilnika o Tehničkom Pregledu Vozila (PTPV)
    </h2>
    <p style='font-size:18px; text-align: left;'>
        🛈 Aplikacija pretražuje tekstualni fajl i na osnovu pretrage prikazuje broj člana i stranu iz PDF verzije pravilnika. Klikom na pdf fajl isti se otvara u posebnom tabu.<br>
...
st.write('🛈 Primer načina pretrage: Unosom reči npr. :orange["*svetlo*"] aplikacija vraća više mogućih rezultata, međutim ako se unese :orange["*kratko svetlo*"] dobija se jedan rezultat.')
...
with col2:
    ...
    if st.session_state.search_query:
        ...
        if matching_lines:
            st.markdown(
                "<h3 style='color: #0077b6; margin-top: -18px; font-size: 17px;'>🔍 Rezultati pretrage:</h3>",
                unsafe_allow_html=True
            )
            for match in matching_lines:
                ...
                if acronym and page_number:
                    pdf_links = {
                        "PPMV": "https://cdn.jsdelivr.net/gh/Sale976/pravilnik/ppmv.pdf",
                        "PTPV": "https://cdn.jsdelivr.net/gh/Sale976/pravilnik/potp.pdf"
                    }
                    pdf_url = pdf_links.get(acronym)
                    if pdf_url:
                        # 💡 IZMENA: Dodat parametar &zoom=125 nakon parametra za stranicu
                        full_url = f"{pdf_url}#page={page_number}&zoom=125"
                        file_link = (
                            f"<a href='{full_url}' target='_blank' "
                            f"title='Otvori PDF na strani {page_number} sa zumom 125%' "
